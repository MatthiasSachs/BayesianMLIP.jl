...
function step!(mhsampler::SimpleMHsampler, m::StatisticalModel)
    ...
    σ = 1/log(mhsampler.step)    # Sigma value determines covariance, which decreases  
...
function step!(mhsampler::AdaptiveMHsampler, m::StatisticalModel)
    ...
    if mhsampler.step == 2
        σ = 1/log(2) 
    elseif mhsampler.step > 2 
        σ = log(mhsampler.step-1)/log(mhsampler.step)
    end 

    # Cool down scaling factor of Σ: Should be 1/log(2), 1/log(3), ... but still asymptotically same 
    mhsampler.Σ = σ * mhsampler.Σ
end
